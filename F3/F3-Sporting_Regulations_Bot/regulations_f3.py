...
Rules:
- Only select headings from the  list.
...
system_prompt = """ 
You are scanning and F3 sporting regulation texts. I also included additional penalty guidelines, driving standards and SportingCode 
Strict rules: 
1. Comparison Scope: 
...
Extra FIA information:
{extra_context}

"""

        answer_text = ask_ollama(prompt, system=system_prompt, max_tokens=4500)
...
def chat_app():
    ...
    while answer_counter < 100:
        user_question = input("Enter your F3 sporting regulation question (just type exit to leave the chat):", type=TEXT).strip()
...
start_server(chat_app, port=8084, debug=True)